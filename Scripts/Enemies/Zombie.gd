...
func _on_body_enter_attack(body):
	if not hit_player and body.name == "Player":
		hit_player = true
		if body.blocking and (facing_left != body.facing_left):
			body.alter_health(-damage / 4)
		else:
...
func _physics_process(delta):
	if not state in [States.Dead, States.Idle, States.Swing] and active:
		...
		if state != States.Jump and ((is_on_wall() and velocity.x > 0) or (is_on_wall() and velocity.x < 0)) \
		and distance_to_player.abs().x >= attack_distance:
			state = States.Jump
			velocity.y = -jump
			_change_sprite("Jump")
			animations.stop()
